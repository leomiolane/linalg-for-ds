...
Let $a_1, \dots, a_n \in \R^d$ be $n$ points in $d$ dimensions. We assume that these points are centered, meaning that 
$$
\sum_{i=1}^n a_i = 0.
$$
...
$$
over the unit sphere $\mathbb{S}_n \defeq \{x \in \R^n \, | \, \|x\|=1 \}$.
...
Assume that the dataset is centered (otherwise subtract the mean $\mu = \frac{1}{n} \sum_{i=1}^n a_i$ to all the points). As we have seen in Section \ref{sec:maximal_variance} above, the $k$ first right-singular vectors $v_1, \dots, v_k$ gives the $k$ first orthogonal direction along which the variance of the dataset is maximal. 

Hence, it is very natural to project the original dataset $a_1, \dots, a_k$ on these directions $\Span(v_1, \dots, v_k)$: we are going to represent each point $a_i$ by the coordinates of its projection, in the basis $v_1, \dots, v_k$. 
...
The dimension $k$ of the output space can be chosen by looking at the singular values of $A$. Let $\sigma_1 \geq \sigma_2 \geq \cdots \geq \sigma_{\min(n,d)}$ be the singular values of $A$. As seen in Section \ref{sec:maximal_variance}, the variance of the dataset along the direction $v_i$ is $\sigma_i^2$.
