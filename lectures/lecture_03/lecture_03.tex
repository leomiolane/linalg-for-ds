...
%\begin{proof}
	%In order to prove \eqref{eq:rank} it suffices to show (since columns and rows are playing exchangeable roles) that $\rank(r_1, \dots, r_n) \leq \rank(c_1, \dots, c_m)$.
	%Let $k \defeq \rank(r_1, \dots, r_n)$ and $(x_1, \dots, x_k)$ be a basis of $\Span(r_1, \dots, r_n)$. We will prove that
	%\begin{equation}\label{eq:goal_proof}
	%(M x_1, \dots, M x_k) \ \text{is linearly independent}.
	%\end{equation}
	%The result follows. Indeed $(M x_1, \dots, M x_k)$ is then a linearly independent family of $k$ vectors of $\Im(M) = \Span(c_1, \dots, c_m)$: this implies that $\rank(c_1, \dots, c_m) = \dim(\Span(c_1, \dots, c_m)) \geq k = \rank(r_1, \dots, r_n)$.

	%It remains to prove \eqref{eq:goal_proof}. Let $\alpha_1, \dots, \alpha_k \in \R$ such that $\alpha_1 M x_1 + \cdots + \alpha_k M x_k = 0$. We will show that in such case the $\alpha_i$ are all zero. Define $v \defeq \alpha_1 x_1 + \cdots + \alpha_k x_r$. We have by linearity
	%$$
	%M v = M (\alpha_1 x_1 + \cdots + \alpha_k x_k) = \alpha_1 M x_1 + \cdots + \alpha_k M x_k = 0.
	%$$
	%Since the $i^{\rm th}$ coordinate of $Mv$ is equal to $(Mv)_i = r_i \cdot v$, we get that $v$ is orthogonal to all the $r_i$, and therefore to $\Span(r_1, \dots, r_n)$. Notice now that $v \in \Span(x_1, \dots, x_r) = \Span(r_1, \dots, r_n)$ by construction. The vector $v$ is orthogonal to itself, hence $\alpha_1 x_1 + \cdots + \alpha_k x_k = v= 0$. Recall that the family $(x_1, \dots, x_k)$ is linearly independent (because it is a basis) so $\alpha_1 = \cdots = \alpha_k = 0$.
%\end{proof}
