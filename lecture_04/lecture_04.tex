...
\begin{definition}[Orthogonal projection and distance to a subspace]
	Let $S$ be a subspace of $V = \R^n$. The \emph{orthogonal projection} of a vector $x$ onto $S$ is defined as the vector $P_S(x)$ is $S$ that minimizes the distance to $x$:
	$$
...
\begin{proposition}
	Let $S$ be a subspace of $V = \R^n$ and let $(v_1, \dots, v_k)$ be a basis of $S$. Then for all $x \in V$,
	\begin{itemize}
...
\section{Gram-Schmidt orthogonalisation method}
